...
public class GrafoMA implements Grafo {
	 ...
	 * BufferedReader (a mí no se me ha ocurrido). Primero le pide al usuario el
	 * número de vértices del grafo (n). Luego crea una matriz de nxn. A
	 * continuación, le pide n veces al usuario que introduzca una línea con n
	 * tokens, que serán los valores de las celdas de la matriz. StringTokenizer
	...
	/**
	 * insertaVertice(int n): se simplifica el método de manera que no deja insertar
	 * vértices si se supera el límite de nodos del grafo maxNodos
	 *
	 */
	...
	public int gradoIn(int j) { //
		int gIn = 0;
		for (int i = 0; i < numVertices; i++) // recorro por filas manteniendo la posición de la columna fija en [j]
			if (matrizAdy[i][j]) {
				gIn++;
	...
	public int gradoOut(int i) {
		int gOut = 0;
		for (int j = 0; j < numVertices; j++) // recorro por columnas, manteniendo la posición de la fila fija en [i]
			if (matrizAdy[i][j]) {
				gOut++;
				if (this.esNoDirigido() && i == j)
					gOut++;
	...
	public void imprimirGrafo() {
		System.out.println("Tamaño máximo del grafo: " + tamanno() + "\n");
